...
old_db = psycopg2.connect("dbname=acmwebsite user=USERNAME_HERE password=PASSWORD_HERE")
old_cur = old_db.cursor()

# open the new db that we're migrating to and a cursor for it
db = psycopg2.connect("dbname=acmdb user=USERNAME_HERE password=PASSWORD_HERE")
cur = db.cursor()
...
for user in response:
    ...
    cur.execute("INSERT INTO users VALUES (%s, %s, %s, %s, %s)", (user_name, display_name, officer_title, '', bio))
...
for image in response:
    ...
    cur.execute("INSERT INTO images VALUES (%s, %s)", (file_id, description))
...
for meeting in response:
    ...
    if title:
        ...
        cur.execute("INSERT INTO meetings VALUES (%s, %s, %s, %s, %s, %s, %s)", (str(uuid.uuid4()), title, description, date, '36000000', location, ''))
...
for presentation in response:
    ...
    cur.execute("INSERT INTO presentations VALUES (%s, %s, %s, %s, %s)", (str(uuid.uuid4()), title, description, date, repo_url))
