...
const configureForm = (form) => {
  const authors = form.querySelector(
    `#${form.getAttribute("data-project-authors-id")}`,
  );
  if (!authors) {
  ...
  authors.innerHTML = `<div id="${authors.id}-control" class="row row-cols-auto align-items-start mx-0 mb-3 gx-1 gy-1">
            <div class="col">
  ...
  authors
    .querySelector(`#${authors.id}-add`)
    .addEventListener("click", () => addAuthor(authors, ""));
  authors
    .querySelector(`#${authors.id}-remove`)
    .addEventListener("click", () => removeAuthor(authors));
...
const populateAuthors = (authors) => {
  let authorValues = JSON.parse(
    authors.getAttribute(`data-project-authors-value`),
  );
