...
router.get(
  ...
  fallible(async (req, res) => {
    const userResp = await db.query("SELECT * FROM users WHERE id = $1", [
      req.params.id,
    ]);
    ...
    if (targetUser) {
      ...
      res.render("profile", {
        ...
        isProfileUser: isUser,
        meetings: meetingsResp.rows,
        projects: projectsResp.rows,
      });
    } else {
...
router.post(
  ...
  fallible(async (req, res) => {
    ...
    await db.query("UPDATE users SET avatar_id = $1 WHERE id = $2", [
      avatarId,
      req.user.id,
    ]);
