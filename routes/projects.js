...
const parseBaseProjectForm = async (req) => {
  ...
  const empty =
    typeof req.body.website === "string" && req.body.website.length === 0;
  if (!empty) {
    try {
  ...
  for (let i = 0; getAuthorValue(i); ++i) {
    ...
    const authorResp = await db.query("SELECT FROM users WHERE id = $1", [
      author,
    ]);
...
router.get(
  ...
  fallible(async (req, res) => {
    const projectsResp = await db.query(
      "SELECT * FROM projects ORDER BY archived, title",
    );
    for (let project of projectsResp.rows) {
      const authorsResp = await db.query(
...
router.post(
  ...
  upload("image"),
  withProjectForm(async (req, res, project) => {
    project.id = uuid.v4();
    if (!project.imageId) {
    ...
    await db.transaction(async (client) => {
      ...
      for (let author of project.authors) {
        await client.query("INSERT INTO project_authors VALUES ($1, $2)", [
          project.id,
          author,
        ]);
      }
...
router.post(
  ...
  withProjectForm(async (req, res, project) => {
    ...
    const imageResp = await db.query(
      "SELECT image_id FROM projects WHERE id = $1",
      [project.id],
    );
    ...
    await db.transaction(async (client) => {
      ...
      await client.query("DELETE FROM project_authors WHERE project_id = $1", [
        project.id,
      ]);
      for (let author of project.authors) {
        await client.query("INSERT INTO project_authors VALUES ($1, $2)", [
