...
router.get(
  ...
  fallible(async (req, res) => {
    let upcomingResp = await db.query(
      "SELECT * FROM meetings WHERE date >= NOW() AND date <= NOW() + INTERVAL '3 weeks' ORDER BY date",
    );
    ...
    let previousResp = await db.query(
      "SELECT * FROM meetings WHERE date <= NOW() ORDER BY date DESC",
    );
    res.render("schedule", {
      title: "Schedule",
      upcoming: upcomingResp.rows,
      previous: previousResp.rows,
    });
  }),
);
...
router.get(
  ...
  fallible(async (req, res) => {
    ...
    const calendar = ical.default({
      name: "Mines ACM",
      description:
        "All of the talks\\, open source meetings\\, and other activities of the Mines ACM Student Chapter.",
      prodId: {
        company: "Mines ACM",
        product: "web",
        language: "EN",
